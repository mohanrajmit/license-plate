...
if __name__ == '__main__':
   images = tf.placeholder(tf.float32, shape=(batch_size, patch_size, patch_size, num_channels), name='rainy_images')  # data
   labels = tf.placeholder(tf.float32, shape=(batch_size, patch_size, patch_size, num_channels), name='ground_truth')  # label
   ...
   validation_data, validation_label =  read_data(input_path, gt_path, input_files, gt_files, patch_size, num_channels, batch_size) #  data for validation
   ...
   with tf.Session(config=config) as sess:
        ...
        for j in range(start_point,iterations):   #  iterations
            ...
            train_data, train_label = read_data(input_path, gt_path, input_files, gt_files, patch_size, num_channels, batch_size) 
